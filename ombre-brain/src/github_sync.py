...
class GitHubSync:
    ...
    async def sync(self, buckets_dir: str, diary_dir: str | None = None) -> dict[str, Any]:
        """同步 buckets_dir + 可选 diary_dir 下所有 .md 到 GitHub。返回结果 dict。

        diary_dir 下的文件会上传到 `{path_prefix}/diary/` 路径下。
        """
        try:
            files = self._collect_files(buckets_dir)
            if diary_dir:
                diary_files = self._collect_files(diary_dir)
                # 重写路径前缀，统一放到 diary/ 子目录下
                for rel_path, content in diary_files.items():
                    files[f"diary/{rel_path}"] = content
            if not files:
                self.last_status = "ok"
                self.last_error = ""
