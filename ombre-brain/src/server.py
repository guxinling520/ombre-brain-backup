...
async def _github_sync_loop(interval_minutes: int) -> None:
    ...
    while True:
        ...
        if not buckets_dir:
            continue
        diary_dir = os.path.join(os.path.dirname(buckets_dir), "diary")
        try:
            result = await inst.sync(buckets_dir, diary_dir)
            if result.get("ok"):
...
# --- Baseline Search / baseline 关键词搜索（替代 vector-memory server）---
from baseline_search import BaselineSearch
baseline_search_engine = BaselineSearch()
# 初始索引在 lifespan 中异步重建，避免阻塞启动
...
async def api_github_sync(request: Request) -> Response:
    ...
    diary_dir = os.path.join(os.path.dirname(buckets_dir), "diary")
    result = await github_sync_instance.sync(buckets_dir, diary_dir)
    return JSONResponse(result)
...
# --- Baseline Search API / baseline 关键词搜索 ---

@mcp.custom_route("/api/archive-search", methods=["GET"])
async def api_archive_search(request: Request) -> Response:
    """BM25 关键词搜索 over baseline/ 文件。替代 vector-memory server 的 /archive-search。
    用法：GET /api/archive-search?q=关键词&limit=5
    """
    from starlette.responses import JSONResponse
    q = request.query_params.get("q", "").strip()
    limit_str = request.query_params.get("limit", "5").strip()
    try:
    ...
    results = baseline_search_engine.search(q, limit=limit)
...
if __name__ == "__main__":
    ...
    if transport in ("sse", "streamable-http"):
        ...
        if transport == "streamable-http":
            # Merge extra tools into main mcp so /mcp exposes all 11 tools
            try:
                mcp._tool_manager._tools.update(mcp_extra._tool_manager._tools)
                logger.info(f"streamable-http 模式：已回灌 {len(mcp_extra._tool_manager._tools)} 个副集工具到 /mcp")
            except AttributeError as e:
                logger.warning(f"FastMCP 内部结构变化，/mcp 仅暴露主集 5 工具：{e}")
            ...
            # Auto-cleanup orphaned MCP sessions after 5 min idle (Claude Code
            # processes exit after each turn, leaving orphaned sessions that
            # accumulate and cause "Transport closed" when evicted).
            mcp.session_manager.session_idle_timeout = 300
            mcp_extra.session_manager.session_idle_timeout = 300
            ...
            async def _combined_lifespan(app):
                async with _main_lifespan(app):
                    async with _extra_lifespan(app):
                        ...
                        # Rebuild baseline search index on startup (background)
                        try:
                            baseline_search_engine.rebuild_index()
                            logger.info("[baseline_search] 索引重建完成")
                        except Exception as _bs_err:
                            logger.warning(f"[baseline_search] 索引重建失败: {_bs_err}")
                        # Start diary sync scheduler as background thread
                        try:
                            from sync_integration import start as _start_sync
                            _start_sync()
                        except Exception as _sync_err:
                            logger.warning(f"[sync] 启动 sync_scheduler 失败: {_sync_err}")
                        yield
                        _stop_tunnel()
        ...
        class _MCPAuthMiddleware:
            ...
            async def __call__(self, scope, receive, send):
                if scope["type"] == "http":
                    ...
                    # Hide OAuth metadata from localhost so clients connect directly
                    if is_local and path == "/.well-known/oauth-protected-resource":
                        await send({"type": "http.response.start", "status": 404, "headers": [
                            [b"content-type", b"application/json"],
                        ]})
                        await send({"type": "http.response.body", "body": b'{"error":"Not Found"}', "more_body": False})
                        return
